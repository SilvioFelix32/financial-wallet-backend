...
export class CognitoStrategy {
  ...
  private async isTokenValid(token: string): Promise<boolean> {
    try {
      ...
      console.info(
        'CognitoStrategy.isTokenValid - Token Expiration:',
        new Date(decoded.exp! * 1000),
      );
      console.info('CognitoStrategy.isTokenValid - Current Time:', new Date());

      const key = await this.getKey(token);
      const { payload } = await jwtVerify(token, key, {
        clockTolerance: 60 * 60 * 3,
      });
      ...
      throw new UnauthorizedException(
        `CognitoStrategy.verifyToken error: ${error}`,
      );
    }
  }
  ...
  private async getKey(token: string) {
    ...
    if (!key) {
      throw new UnauthorizedException(
        'CognitoStrategy.getKey: Key not found in jwks',
      );
    }
  ...
  private async getJwks(): Promise<JSONWebKeySet> {
    try {
      ...
      throw new UnauthorizedException(
        `CognitoStrategy.getJwks error while getting jwks: ${error}`,
      );
    }
  }
}
